...
public class EmbeddedSQL {
   ...
   public static void Query1(EmbeddedSQL esql){
      // Your code goes here.
	try{
         String query = "SELECT S.sname, COUNT(*) AS TotalParts FROM suppliers S, catalog C, parts P WHERE C.sid = S.sid AND P.pid = C.pid GROUP BY S.sname, S.sid";
         int rowCount = esql.executeQuery(query);
         System.out.println ("total row(s): " + rowCount);
      }catch(Exception e){
         System.err.println (e.getMessage());
   ...
   public static void Query2(EmbeddedSQL esql){
      // Your code goes here.
	try{
         String query = "SELECT S.sname, COUNT(*) as TotalParts FROM suppliers S, catalog C, parts P WHERE C.sid = S.sid AND P.pid = C.pid GROUP BY S.sname, S.sid HAVING COUNT(*) >= 3";
         int rowCount = esql.executeQuery(query);
         System.out.println ("total row(s): " + rowCount);
      }catch(Exception e){
         System.err.println (e.getMessage());
   ...
   public static void Query3(EmbeddedSQL esql){
      // Your code goes here.
	try{
         String query = "SELECT S.sname, COUNT(*) as TotalParts FROM Suppliers S, Parts P, Catalog C WHERE P.pid = C.pid AND C.sid = S.sid GROUP BY S.sname, S.sid HAVING EVERY (P.color = 'Green')";
         int rowCount = esql.executeQuery(query);
         System.out.println ("total row(s): " + rowCount);
      }catch(Exception e){
         System.err.println (e.getMessage());
   ...
   public static void Query4(EmbeddedSQL esql){
      // Your code goes here.
	try{
         String query = "CREATE VIEW TEMP as SELECT DISTINCT C.sid FROM catalog C, parts P WHERE C.pid = P.pid AND P.color = 'Red' INTERSECT SELECT DISTINCT C1.sid FROM catalog C1, parts P1 WHERE C1.pid = P1.pid AND P1.color = 'Green'; SELECT S.sname, Max(C.cost) as MostExpensive FROM TEMP, catalog C, suppliers S WHERE TEMP.sid = c.sid AND C.sid = S.sid GROUP BY S.sname, S.sid";
         int rowCount = esql.executeQuery(query);
         System.out.println ("total row(s): " + rowCount);
      }catch(Exception e){
         System.err.println (e.getMessage());
   ...
   public static void Query5(EmbeddedSQL esql){
      // Your code goes here.
      try{
         String query = "SELECT P.Pname FROM parts P, catalog C WHERE C.pid = P.pid AND cost < ";
         System.out.print("\tEnter cost: $");
         String input = in.readLine();
         query += input;
